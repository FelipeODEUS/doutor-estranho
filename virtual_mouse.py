...
def countFingers(image, hand_landmarks, handNo=0):
	...
	if hand_landmarks:
		...
		pontinhaX = int((landmarks[8].x)*width)
		pontinhaY = int((landmarks[8].y)*height)
		pontonaX = int((landmarks[4].x)*width)
		pontonaY = int((landmarks[4].y)*height)
		cv2.line(image,(pontinhaX, pontinhaY), (pontonaX, pontonaY),(255,0,0),2)

		# Desenhe um CÍRCULO no CENTRO da LINHA entre a PONTA DO DEDO e a PONTA DO POLEGAR
		centrinhoX = int((pontinhaX + pontonaX)/2)
		centrinhoY = int((pontinhaY + pontonaY)/2)
		cv2.circle(image, (centrinhoX, centrinhoY), 2, (0,0,255), 2)
		# Calcule a DISTÂNCIA entre a PONTA DO DEDO e a PONTA DO POLEGAR
		dist = math.sqrt(((pontinhaX-pontonaX)**2)+((pontinhaY-pontonaY)**2))
		print(dist)

		# Defina a posição do mouse na tela em relação ao tamanho da janela de resultado	
		mouseX = (centrinhoX/width)*screen_width
		mouseY = (centrinhoY/height)*screen_height
